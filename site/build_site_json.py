...
"""Build the JSON the NHL Projections site reads, from the lab's own outputs.
    ...
    PYTHONPATH=src python site/build_site_json.py --out dist/data
...
def load_record(path: Path) -> dict:
    empty = {"straightUp": {"w": 0, "l": 0}, "puckLine": {"w": 0, "l": 0, "p": 0}, "totals": {"w": 0, "l": 0, "p": 0}, "forward": None}
    if not path.is_file():
        return empty
    try:
        ...
        return empty
    # The forward-evidence payload is the lab's; only its top-line numbers are lifted, and only when present.
    overall = fe.get("overall") or fe.get("summary") or {}
    if overall:
        empty["forward"] = {
            "opinions": overall.get("opinions", 0), "wagers": overall.get("bets", overall.get("wagers", 0)),
            "roiPct": round(100 * float(overall.get("roi", 0) or 0), 1),
            "ciLow": round(100 * float(overall.get("roi_low", 0) or 0), 1), "ciHigh": round(100 * float(overall.get("roi_high", 0) or 0), 1),
            "clvPct": round(100 * float(overall.get("clv", 0) or 0), 1), "beatClosePct": round(100 * float(overall.get("beat_close_rate", 0) or 0), 1),
        }
    return empty
