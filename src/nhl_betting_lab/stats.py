...
class RoiInterval:
    ...
    def as_row(self, label: str) -> str:
        ...
        return (
            ...
            f"{'yes' if self.includes_zero else 'no'} |"
        )


ROI_TABLE_HEADER = (
    "| Market | Bets | Profit | ROI | 95% interval | Includes zero |\n"
    "|:-------|-----:|-------:|----:|:-------------|:--------------|"
)


def roi_interval(
    returns: Sequence[float], *, wins: int = 0, pushes: int = 0
) -> RoiInterval:
    ...
    if bets == 0:
        return RoiInterval(
            bets=0, staked=0.0, profit=0.0, roi=0.0, low=0.0, high=0.0
        )
    profit = sum(rows)
    staked = float(bets)
    roi = profit / staked
